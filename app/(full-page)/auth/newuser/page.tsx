        ...
        loginService.novoCadastro(usuario)
            .then((response) => {
                ...
                toast.current?.show({
                    severity: 'success',
                    summary: 'Sucesso',
                    detail: 'Usuarios cadastrado com sucesso!',
                    life: 3000
                })
            }).catch((error) => {
                ...
                >
                    <div className="w-full surface-card py-8 px-5 sm:px-8" style={{ borderRadius: '53px' }}>
                        ...
                        <div>
                            ...
                            <InputText id="nome" type="text" placeholder="Digite seu nome..." className="w-full md:w-30rem mb-5" style={{ padding: '1rem' }} value={usuario.nome}
                                onChange={(e) => onInputChange(e, 'nome')} />

                            <label htmlFor="login" className="block text-900 text-xl font-medium mb-2">
                                Login
                            </label>
                            <InputText id="login" type="text" placeholder="Digite seu login..." className="w-full md:w-30rem mb-5" style={{ padding: '1rem' }} value={usuario.login}
                                onChange={(e) => onInputChange(e, 'login')} />

                            <label htmlFor="email" className="block text-900 text-xl font-medium mb-2">
                                Email
                            </label>
                            <InputText id="email" type="text" placeholder="Digite seu email..." className="w-full md:w-30rem mb-5" style={{ padding: '1rem' }} value={usuario.email}
                                onChange={(e) => onInputChange(e, 'email')} />

                            <label htmlFor="senha" className="block text-900 font-medium text-xl mb-2">
                                Senha
                            </label>
                            <Password inputId="senha" placeholder="Digite a sua senha..." toggleMask className="w-full mb-5" inputClassName="w-full p-3 md:w-30rem" value={usuario.senha}
